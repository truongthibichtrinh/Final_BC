<?php
	...
	class db_class extends db_connect{
		...
		public function add_user($username,$password,$name,$role){
			$query=$this->conn->prepare("INSERT INTO `user` (`username`, `password`, `name`, `role`) VALUES(?, ?, ?, ?)") or die($this->conn->error);
			$query->bind_param("ssss", $username, $password, $name, $role);
		...
		public function update_user($user_id,$username,$password,$name,$role){
			$query=$this->conn->prepare("UPDATE `user` SET `username`=?, `password`=?, `name`=?, `role`=? WHERE `user_id`=?") or die($this->conn->error);
			$query->bind_param("ssssi", $username, $password, $name, $role, $user_id);
